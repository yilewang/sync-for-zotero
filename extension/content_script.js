...
async function attachImages(imageDataUrls) {
  ...
  for (const dataUrl of imageDataUrls) {
    ...
    const targets = [dropTarget, document.body];
    ...
    // Fallback: try the file input directly
...
async function attachPDF(pdfBase64, pdfFilename) {
  ...
  const fileInput = document.querySelector("input[type='file'][accept*='pdf'], input[type='file']");
  if (fileInput) {
    const nativeSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "files")?.set;
    if (nativeSetter) {
      nativeSetter.call(fileInput, dt.files);
