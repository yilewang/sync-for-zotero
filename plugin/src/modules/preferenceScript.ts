...
export async function registerPrefsScripts(_window: Window | undefined | null) {
  ...
  const rerender = () => {
    ...
    groups.forEach((group, groupIndex) => {
      ...
      // [webchat] Auth mode helper text
      const authModeHelperText =
        group.authMode === "webchat"
          ? t("Relay queries to a web-based LLM chat via the sync-for-zotero browser extension.")
      ...
      const selectedPresetId: ProviderPresetId =
        group.authMode === "codex_auth" || group.authMode === "copilot_auth" || group.authMode === "webchat" // [webchat]
          ? "customized"
      ...
      const isCustomizedPreset =
        group.authMode !== "codex_auth" &&
        group.authMode !== "copilot_auth" &&
        group.authMode !== "webchat" && // [webchat]
        selectedPresetId === "customized";
      ...
      if (group.authMode !== "codex_auth" && group.authMode !== "copilot_auth" && group.authMode !== "webchat") { // [webchat]
      ...
      // [webchat] Minimal layout for webchat — just auth mode, target info, and models
      if (group.authMode === "webchat") {
        const webchatInfoWrap = el(doc, "div", "display: flex; flex-direction: column;");
        const webchatInfoLabel = el(doc, "label", LABEL_STYLE, t("Web Chat Target"));
        const webchatInfoText = el(
          doc,
          "span",
          HELPER_STYLE,
